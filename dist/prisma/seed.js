...
async function main() {
    ...
    const posts = await Promise.all(users.map((user, i) => prisma.post.create({
        data: {
            userId: user.id,
    ...
    const allComments = await Promise.all(posts.flatMap((post) => users.slice(0, 2).map((user) => prisma.comment.create({
        data: {
            postId: post.id,
            userId: user.id,
